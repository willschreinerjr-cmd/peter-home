...
function DeleteBtn({ onClick }) {
  return (
    <button onClick={onClick} aria-label="Remove" style={{ flexShrink:0, width:28, height:28, borderRadius:''50%'', border:''1px solid rgba(248,113,113,0.22)'', background:''rgba(248,113,113,0.06)'', color:''#f87171'', fontSize:''1rem'', cursor:''pointer'', display:''flex'', alignItems:''center'', justifyContent:''center'', lineHeight:1 }}>×</button>
  )
}
...
function TokenSection({ token, onSave }) {
  ...
  return (
    <Card>
      <div onClick={()=>setOpen(e=>!e)} style={{ padding:''12px 16px'', display:''flex'', alignItems:''center'', justifyContent:''space-between'', cursor:''pointer'', userSelect:''none'' }}>
...
function PushButton({ photos, workouts, token }) {
  ...
  return (
    <div style={{ margin:''0 14px'', display:''flex'', flexDirection:''column'', gap:8 }}>
      ...
      {st===''err'' && <p style={{ fontSize:''0.70rem'', color:''#f87171'', textAlign:''center'' }}>✗ {err}</p>}
  ...
  return (
    <div style={{ position:''relative'', zIndex:3, height:''100%'', overflowY:''auto'', overflowX:''hidden'', WebkitOverflowScrolling:''touch'', cursor:''auto'' }}>
      <div style={{ padding:''18px 20px 14px'', background:''rgba(10,5,1,0.75)'', borderBottom:`1px solid ${bdr}`, backdropFilter:''blur(14px)'', WebkitBackdropFilter:''blur(14px)'', position:''sticky'', top:0, zIndex:10 }}>
        <h1 style={{ fontSize:''1.45rem'', fontWeight:800, color:cream, letterSpacing:''-0.02em'', lineHeight:1.1 }}>⚙️ Dashboard Editor</h1>
        <p style={{ fontSize:''0.70rem'', color:dim, marginTop:2 }}>{settings.ownerName}&rsquo;s Home · Changes push to the big screen</p>
      </div>
